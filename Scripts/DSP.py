...
from Scripts.feature_class import features
...
def classify_cough(x, fs, model, scaler):
    ...
    try:
        ...
        feature_values_scaled = scaler.transform(
            np.array(feature_values_vec).reshape(1, -1)
        )
...
def preprocess_cough(x, fs, cutoff=6000, normalize=True, filter_=True, downsample=True):
    ...
    *: mandatory input
    ...
    if filter_:
        b, a = butter(
            4, fs_downsample / fs, btype="lowpass"
        )  # 4th order butter lowpass filter
        x = filtfilt(b, a, x)
    if downsample:
